...
shaka.util.BjsnBoxStripper = class {
  ...
  static stripBjsnBox(segmentData) {
    try {
      ...
      if (!bjsnBoxInfo) {
        shaka.log.warning(
            '  ⚠️ BJSN box detection failed, returning original data');
        return segmentData;
      }
      ...
      shaka.log.info('  📏 Output segment size:', strippedData.length,
          'bytes');
      ...
      if (!shaka.util.BjsnBoxStripper.validateMp4Structure_(strippedData)) {
        shaka.log.warning(
            '  ⚠️ Stripped segment validation failed, returning original ' +
            'data');
        return segmentData;
      }
   ...
   * @param {{start: number, size: number, headerSize: number}} boxInfo - Box to
   *     remove
    ...
    try {
      ...
      while (offset < segmentData.length) {
        ...
        if (boxType === 'bjsn' || boxType === 'moov' || boxType === 'ftyp') {
          ...
          shaka.log.v2('🔧 BJSN BOX STRIPPER: Skipping', boxType,
      ...
      shaka.log.warning(
          '🔧 BJSN BOX STRIPPER: Failed to strip BJSN and MOOV boxes:',
          error);
