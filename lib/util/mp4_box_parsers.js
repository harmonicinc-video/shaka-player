...
shaka.util.Mp4BoxParsers = class {
  ...
  /**
   * Parses a HDLR box - ENHANCED FOR BJSN SUPPORT
   * @param {!shaka.util.DataViewReader} reader
   * @return {!shaka.util.ParsedHDLRBox}
   */
  static parseHDLR(reader) {
    const initialPosition = reader.getPosition();
    const remainingLength = reader.getLength() - initialPosition;
    
    // Log detailed debugging info
    if (typeof shaka !== 'undefined' && shaka.log) {
      shaka.log.info('🔍 HDLR: reader position:', initialPosition, 'remaining:', remainingLength);
      
      // Show first 16 bytes for debugging
      const debugBytes = reader.readBytes(Math.min(16, remainingLength));
      shaka.log.info('🔍 HDLR: first 16 bytes:', 
                     Array.from(debugBytes).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' '));
      reader.seek(initialPosition);
    }
    
    // CRITICAL FIX: Based on actual file analysis, the handler type is at offset 4, not 8!
    // HDLR payload structure (after version/flags removed):
    // 4 bytes: pre_defined (usually 0)
    // 4 bytes: handler_type (fourcc) ← ACTUAL LOCATION!
    // 4 bytes: reserved (usually 0)  
    // remaining: name (null-terminated string)
    
    try {
      // Skip 4 bytes (pre_defined only)
      reader.skip(4);
      
      // Read handler type as 4 bytes at offset 4
      if (reader.getPosition() + 4 <= reader.getLength()) {
        const handlerTypeBytes = reader.readBytes(4);
        ...
        if (typeof shaka !== 'undefined' && shaka.log) {
          shaka.log.info('🔍 HDLR: handler type bytes:', 
                         Array.from(handlerTypeBytes).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' '),
                         '→', handlerType);
        }
        
        // Validate handler type (should be printable ASCII)
        if (handlerType.match(/^[a-zA-Z]{4}$/)) {
          return {handlerType};
        } else {
          // If the 4-byte read didn't work, try different offsets
          reader.seek(initialPosition);
          reader.skip(8);
          const handlerTypeOriginal = reader.readTerminatedString();
          
          if (typeof shaka !== 'undefined' && shaka.log) {
            shaka.log.info('🔍 HDLR: fallback to terminated string:', handlerTypeOriginal);
          }
          
          return {handlerType: handlerTypeOriginal};
        }
      } else {
        if (typeof shaka !== 'undefined' && shaka.log) {
          shaka.log.warning('🔍 HDLR: insufficient data for handler type');
        }
        return {handlerType: ''};
      }
    } catch (error) {
