 ...
 *
 * Dynamically detects codec information from MP4 segments after BJSN box
 * stripping using existing Shaka Player MP4 parsing infrastructure for
 * accurate MIME type generation.
 *
 * FEATURES:
...
shaka.util.BjsnCodecDetector = class {
   ...
   *
   * @param {!Uint8Array} segmentData - Stripped MP4 segment data (without
   *   BJSN box)
   * @return {!shaka.util.BjsnCodecDetector.CodecInfo} Detected codec
   *   information
   * @export
   */
  static detectCodecsFromSegment(segmentData) {
    try {
      ...
      const isInitSegment = shaka.util.BjsnCodecDetector.hasBox_(
          segmentData, 'moov');

      if (isInitSegment) {
        return shaka.util.BjsnCodecDetector.detectCodecsFromInitSegment_(
            segmentData);
      } else {
        // For media segments, we need to parse moof/traf structure
        return shaka.util.BjsnCodecDetector.detectCodecsFromMediaSegment_(
            segmentData);
      }
    } catch (error) {
  ...
  static detectCodecsFromInitSegment_(segmentData) {
    ...
    try {
      ...
      for (let i = 0; i < tracks.length; i++) {
        ...
        const codecData = shaka.util.BjsnCodecDetector.parseSTSD_(
            track.stsdData, track.trackType);
      ...
      // Generate MIME type
      codecInfo.mimeType = shaka.util.BjsnCodecDetector.generateMimeType_(
          codecInfo);
  ...
  static parseSTSD_(stsdData, trackType) {
    ...
    try {
      ...
      for (let i = 0; i < entryCount; i++) {
        ...
        const entryEnd = entryContentStart + entrySize - 8; // Subtract size
        // and format fields
        ...
        if (trackType === 'vide' &&
            shaka.util.BjsnCodecDetector.isVideoFormat_(formatString)) {
          result.video = shaka.util.BjsnCodecDetector.parseVideoSampleEntry_(
            ...
            shaka.util.BjsnCodecDetector.isAudioFormat_(formatString)) {
  ...
  static parseVideoSampleEntry_(reader, format, contentStart, contentSize) {
    try {
      ...
      if (!codecString) {
        codecString = shaka.util.BjsnCodecDetector.getDefaultVideoCodec_(
            format);
      }
  ...
  static parseAudioSampleEntry_(reader, format, contentStart, contentSize) {
    try {
      ...
      // Fallback to default if parsing failed
      if (!codecString) {
        codecString = shaka.util.BjsnCodecDetector.getDefaultAudioCodec_(
            format);
      }
  ...
  static parseAvcC_(data) {
        ...
        if (boxSize >= 12 && i + boxSize <= data.length) {
          ...
          if (avcCContentStart + 4 <= data.length) {
            ...
            const codecString = 'avc1.' +
              avcProfileIndication.toString(16).padStart(2, '0')
                  .toUpperCase() +
              profileCompatibility.toString(16).padStart(2, '0')
                  .toUpperCase() +
              avcLevelIndication.toString(16).padStart(2, '0')
                  .toUpperCase();
    ...
    // Method 2: Fallback - look for 'avcC' signature without strict box
    // parsing. This handles cases where we might have partial data or
    // different structure
    for (let i = 0; i < data.length - 8; i++) {
      if (data[i] === 0x61 && data[i+1] === 0x76 && data[i+2] === 0x63 &&
          data[i+3] === 0x43) {
        // Try different content start positions
        const possibleStarts = [i + 4, i + 8]; // After fourcc, or after
        // size+fourcc
  ...
  static parseHvcC_(data) {
    // Look for 'hvcC' box
    for (let i = 0; i < data.length - 8; i++) {
      if (data[i] === 0x68 && data[i+1] === 0x76 && data[i+2] === 0x63 &&
          data[i+3] === 0x43) {
        // For now, return a standard H.265 codec string
        // Full hvcC parsing is complex and would require more detailed
        // implementation
  ...
  static parseESDS_(data) {
    ...
    for (let i = 0; i <= data.length - 12; i++) {
      ...
      if (i + 8 < data.length &&
        ...
        // Validate box size
        if (boxSize >= 12 && i + boxSize <= data.length) {
          return shaka.util.BjsnCodecDetector.parseESDSWithSharedParser_(
              data, i + 8, boxSize - 8);
        }
      }
    }
    ...
    for (let i = 0; i < data.length - 8; i++) {
      if (data[i] === 0x65 && data[i+1] === 0x73 && data[i+2] === 0x64 &&
          data[i+3] === 0x73) {
        return shaka.util.BjsnCodecDetector.parseESDSWithSharedParser_(
            data, i + 4, Math.min(50, data.length - i - 4));
      }
    }
  ...
  static parseESDSWithSharedParser_(data, contentStart, contentSize) {
    try {
      ...
      const reader = new shaka.util.DataViewReader(
          esdsData, shaka.util.DataViewReader.Endianness.BIG_ENDIAN);
      ...
      return shaka.util.BjsnCodecDetector.parseESDSFallback_(
          data, contentStart, contentSize);
    }
  }
  ...
  static isVideoFormat_(format) {
    const videoFormats = ['avc1', 'avc3', 'hvc1', 'hev1', 'vp09', 'av01',
      'mp4v'];
  ...
  static createDetectionResult(segmentUrl, codecInfo) {
    return {
      ...
      codecInfo: codecInfo,
      timestamp: Date.now(),
      isValid: shaka.util.BjsnCodecDetector.validateCodecSupport(
          codecInfo.mimeType),
    };
  }
};
 ...
 *   How the codec was detected ('init-segment', 'media-segment-fallback',
 *   'fallback')
 * @exportDoc
 */
