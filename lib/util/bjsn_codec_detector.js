...
shaka.util.BjsnCodecDetector = class {
  ...
  static detectCodecsFromInitSegment_(segmentData) {
    ...
    const parser = new shaka.util.Mp4Parser()
      ...
      .box('trak', (box) => {
        ...
        shaka.log.info('    📦 Found trak box, created track #', tracks.length);
        shaka.util.Mp4Parser.children(box);
      })
      .box('tkhd', (box) => {
        shaka.log.info('    📦 Found tkhd box');
      })
      .box('mdia', (box) => {
        shaka.log.info('    📦 Found mdia box');
        shaka.util.Mp4Parser.children(box);
      })
      .box('mdhd', (box) => {
        shaka.log.info('    📦 Found mdhd box');
      })
      .fullBox('hdlr', (box) => {
        shaka.log.info('    📦 Found hdlr box');
        shaka.log.info('    📍 Box reader position:', box.reader.getPosition());
        shaka.log.info('    📏 Box reader length:', box.reader.getLength());
        shaka.log.info('    📄 Box version:', box.version, 'flags:', box.flags);
        
        try {
          // Use the enhanced parseHDLR function
          const hdlr = shaka.util.Mp4BoxParsers.parseHDLR(box.reader);
          shaka.log.info('    🔍 HDLR box parsed:', hdlr);
          if (currentTrack && hdlr && hdlr.handlerType) {
            ...
            shaka.log.warning('    ⚠️  HDLR parsing failed or no current track');
            shaka.log.warning('       - currentTrack:', !!currentTrack);
            shaka.log.warning('       - hdlr:', hdlr);
          }
        } catch (error) {
      ...
      .box('minf', (box) => {
        shaka.log.info('    📦 Found minf box');
        shaka.util.Mp4Parser.children(box);
      })
      .box('stbl', (box) => {
        shaka.log.info('    📦 Found stbl box');
        shaka.util.Mp4Parser.children(box);
      })
      .fullBox('stsd', (box) => {
        shaka.log.info('    📦 Found stsd box');
        if (currentTrack) {
    ...
    try {
      ...
      shaka.log.info('  🔍 Processing', tracks.length, 'tracks for codec detection');
      
      for (let i = 0; i < tracks.length; i++) {
        const track = tracks[i];
        shaka.log.info('    📋 Track', i + 1, '- Type:', track.trackType, 'STSD:', !!track.stsdData);
        
        if (!track.trackType || !track.stsdData) {
          shaka.log.warning('    ⚠️ Incomplete track data, skipping track', i + 1);
          shaka.log.warning('      - trackType:', track.trackType);
          shaka.log.warning('      - stsdData:', !!track.stsdData);
          continue;
        }
  ...
  static parseAvcC_(data) {
    shaka.log.info('        🔍 Searching for avcC box in', data.length, 'bytes');
    ...
    for (let i = 0; i <= data.length - 12; i++) {
      ...
      if (i + 8 < data.length && 
        ...
        shaka.log.info('        📦 Found avcC box at offset:', i, 'size:', boxSize);
        ...
        if (boxSize >= 12 && i + boxSize <= data.length) {
          ...
          if (avcCContentStart + 4 <= data.length) {
            ...
            shaka.log.info('        📊 avcC config: version=', configurationVersion,
                         'profile=0x' + avcProfileIndication.toString(16),
                         'compat=0x' + profileCompatibility.toString(16),
                         'level=0x' + avcLevelIndication.toString(16));
    ...
    for (let i = 0; i < data.length - 8; i++) {
      if (data[i] === 0x61 && data[i+1] === 0x76 && data[i+2] === 0x63 && data[i+3] === 0x43) {
        shaka.log.info('        📦 Found avcC signature at offset:', i, '(fallback method)');
        ...
        for (const contentStart of possibleStarts) {
          if (contentStart + 4 <= data.length) {
            ...
            if (configurationVersion === 1 && avcProfileIndication > 0 && avcLevelIndication > 0) {
              shaka.log.info('        📊 avcC config (fallback): version=', configurationVersion,
                           'profile=0x' + avcProfileIndication.toString(16),
                           'compat=0x' + profileCompatibility.toString(16),
                           'level=0x' + avcLevelIndication.toString(16));
              ...
              shaka.log.info('        🎯 H.264 codec string (fallback):', codecString);
              return codecString;
            }
          }
        }
    ...
    shaka.log.warning('        ⚠️  No valid avcC box found');
    return null;
  }
  ...
  /**
   * Parse ESDS box to extract AAC codec string - IMPROVED VERSION
   * 
  ...
  static parseESDS_(data) {
    shaka.log.info('        🔍 Searching for esds box in', data.length, 'bytes');
    ...
    for (let i = 0; i <= data.length - 12; i++) {
      ...
      if (i + 8 < data.length && 
        ...
        shaka.log.info('        📦 Found esds box at offset:', i, 'size:', boxSize);
        
        // Validate box size
        if (boxSize >= 12 && i + boxSize <= data.length) {
          return shaka.util.BjsnCodecDetector.parseESDSContent_(data, i + 8, boxSize - 8);
        }
      }
    }
    ...
    for (let i = 0; i < data.length - 8; i++) {
      if (data[i] === 0x65 && data[i+1] === 0x73 && data[i+2] === 0x64 && data[i+3] === 0x73) {
        shaka.log.info('        📦 Found esds signature at offset:', i, '(fallback method)');
        return shaka.util.BjsnCodecDetector.parseESDSContent_(data, i + 4, Math.min(50, data.length - i - 4));
      }
    }
  ...
  /**
   * Parse ESDS content to extract AudioObjectType
   * 
   ...
   * @private
   */
  static parseESDSContent_(data, contentStart, contentSize) {
    try {
      // ESDS parsing is complex, but we can use a simplified approach
      ...
      shaka.log.info('        🔍 Parsing ESDS content from', searchStart, 'to', searchEnd);
      ...
      shaka.log.warning('        ⚠️  ESDS content parsing failed:', error);
