...
shaka.media.PreloadManager = class extends shaka.util.FakeEventTarget {
  ...
  async parseManifestInner_() {
    ...
    if (!this.parser_) {
      // Check if BJSN parsing is enabled in configuration
      if (this.config_.manifest.bjsn && this.config_.manifest.bjsn.enabled) {
        // Use BJSN parser when explicitly configured
        this.parserFactory_ = shaka.media.BjsnManifestParser.factory;
        this.parser_ = this.parserFactory_();
        shaka.log.info('Using BJSN manifest parser ' +
