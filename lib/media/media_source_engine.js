...
shaka.media.MediaSourceEngine = class {
  ...
  append_(contentType, data, timestampOffset, stream) {
    ...
    // BJSN DEBUG: Log detailed info before feeding data to MSE
    shaka.log.info('🔥 BJSN/MSE DEBUG: About to append data to MSE');
    shaka.log.info('  📝 ContentType:', contentType);
    shaka.log.info('  📏 Data size:', data.byteLength, 'bytes');
    shaka.log.info('  ⏰ Timestamp offset:', timestampOffset);
    shaka.log.info('  🎬 Stream info:', {
      id: stream.id,
    ...
    // BJSN STRIPPING: Check if this is a BJSN segment that needs stripping
    let processedData = data;
    if (data && data.byteLength > 0) {
      const uint8Data = shaka.util.BufferUtils.toUint8(data);

      // Check if this is a BJSN segment
      if (shaka.util.BjsnBoxStripper.needsStripping(uint8Data)) {
        shaka.log.info('  🔧 BJSN segment detected, stripping BJSN box...');

        // Get detailed info about the BJSN box
        const bjsnInfo = shaka.util.BjsnBoxStripper.getBjsnBoxInfo(uint8Data);
        shaka.log.info('  📋 BJSN box info:', bjsnInfo);

        // Strip the BJSN box
        const strippedData = shaka.util.BjsnBoxStripper.stripBjsnBox(uint8Data);
        processedData = strippedData.buffer;

        shaka.log.info('  ✅ BJSN box stripped successfully');
        shaka.log.info('  📏 Original size:', data.byteLength, 'bytes');
        shaka.log.info('  📏 Stripped size:', processedData.byteLength, 'bytes');
        const reduction = data.byteLength - processedData.byteLength;
        shaka.log.info('  📊 Size reduction:', reduction, 'bytes');
      } else {
        shaka.log.info('  ✅ No BJSN box detected, using original data');
      }
    }

    const sourceBuffer = this.sourceBuffers_.get(contentType);
    if (sourceBuffer) {
      shaka.log.info('  📺 SourceBuffer state:', {
        updating: sourceBuffer.updating,
        readyState: this.mediaSource_.readyState,
      ...
      // Log buffered ranges before append
      try {
        ...
        shaka.log.info('  📊 Buffered ranges before append:', ranges);
      } catch (e) {
        shaka.log.info('  📊 Cannot read buffered ranges:', e.message);
      }
    } else {
      shaka.log.error(
          '  ❌ No SourceBuffer found for contentType:', contentType);
    }

    // This will trigger an 'updateend' event.
    this.sourceBuffers_.get(contentType).appendBuffer(processedData);

    shaka.log.info('  ✅ Data successfully passed to MSE appendBuffer()');
  }
  ...
  onUpdateEnd_(contentType) {
    // BJSN DEBUG: Log updateend event
    shaka.log.info('🔥 BJSN/MSE DEBUG: SourceBuffer updateend event');
    shaka.log.info('  📝 ContentType:', contentType);

    const sourceBuffer = this.sourceBuffers_.get(contentType);
    if (sourceBuffer) {
      // Log buffered ranges after append
      try {
        const buffered = sourceBuffer.buffered;
        const ranges = [];
        for (let i = 0; i < buffered.length; i++) {
        ...
        shaka.log.info('  📊 Buffered ranges after append:', ranges);
      } catch (e) {
        shaka.log.info('   📊 Cannot read buffered ranges:', e.message);
      }
    }
