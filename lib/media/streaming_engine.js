...
shaka.media.StreamingEngine = class {
  ...
  switchInternal_(stream, clearBuffer, safeMargin, force, adaptation) {
      ...
      if (mediaState.stream.closeSegmentIndex) {
        if (mediaState.performingUpdate) {
          ...
          if (!this.deferredCloseSegmentIndex_.has(oldStreamTag)) {
            ...
            // closeSegmentIndex() operation for the old stream once the
            // ongoing update has finished, then immediately create a new
            // segment index.
    ...
    const switchingMuxedAndAlternateAudio =
        mediaState.stream.isAudioMuxedInVideo !=
        stream.isAudioMuxedInVideo;
  ...
  async initStreams_(segmentPrefetchById) {
    ...
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Current variant:',
    ...
    for (const type of streamsByType.keys()) {
      ...
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Processing stream type:',
          type);
      ...
      if (!this.mediaStates_.has(type)) {
        shaka.log.info(
            '🔥 BJSN/StreamingEngine DEBUG: Creating media state for type:',
            type);
        ...
        shaka.log.info(
            '🔥 DEBUG: Scheduling initial update for type:',
            type);
        this.scheduleUpdate_(mediaState, 0);
      } else {
        shaka.log.info(
            '🔥 DEBUG: Media state already exists for type:',
            type);
      }
    }
  }
  ...
  updateDuration() {
    ...
    if (duration < Infinity) {
      ...
      if (isInfiniteLiveStreamDurationSupported) {
        if (this.updateLiveSeekableRangeTimer_) {
          this.updateLiveSeekableRangeTimer_.start(
              /* seconds= */ 1, /* recurring= */ true);
        }
  ...
  async onUpdate_(mediaState) {
    ...
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: onUpdate_() called for type:',
        mediaState.type);
    ...
    if (mediaState.performingUpdate || (mediaState.updateTimer == null)) {
      shaka.log.info(
          '🔥 DEBUG: onUpdate_() early return - invalid state');
      return;
    }
    ...
    if (mediaState.clearingBuffer) {
      shaka.log.info(
          '🔥 DEBUG: onUpdate_() early return - clearing buffer');
      return;
    }

    mediaState.updateTimer = null;
    shaka.log.info(
        '🔥 DEBUG: onUpdate_() proceeding with update process');
  ...
  update_(mediaState) {
    ...
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: update_() called for type:',
        mediaState.type);
    shaka.log.info('  🎬 Stream has isAudioMuxedInVideo:',
        mediaState.stream.isAudioMuxedInVideo);
    ...
    if (shaka.media.StreamingEngine.isEmbeddedText_(mediaState)) {
      shaka.log.info(
          '🔥  DEBUG: Embedded text detected, setting CC ID');
    ...
    if (mediaState.stream.isAudioMuxedInVideo &&
      ...
      shaka.log.v2(logPrefix, 'skipping update for audio muxed in video');
      return null;
    }
    ...
    if (!this.playerInterface_.mediaSourceEngine.isStreamingAllowed() &&
      ...
      shaka.log.v2(logPrefix, 'Holding update');
    ...
    const bufferingGoal = unscaledBufferingGoal * this.bufferingScale_;
    shaka.log.v2(logPrefix, 'bufferingGoal=' + bufferingGoal);
    ...
    shaka.log.info(
        '🔥 BJSN/StreamingEngine DEBUG: Getting segment reference needed');
    ...
    if (timeNeeded >= minTimeNeeded + maxRunAhead) {
      ...
      shaka.log.info(
          '🔥 BJSN/StreamingEngine DEBUG: Waiting for other streams to buffer');
    ...
    shaka.log.info(
        '🔥 BJSN/StreamingEngine DEBUG: About to call fetchAndAppend_');
  ...
  async fetch_(mediaState, reference, streamDataCallback) {
    ...
    shaka.log.info('  ⏰ Start-End time:', reference.startTime, '->',
        reference.endTime);
    ...
    if (!op) {
      shaka.log.info('  🌐 Dispatching network fetch for:',
          reference.getUris()[0]);
  ...
  scheduleUpdate_(mediaState, delay) {
    ...
    shaka.log.info('  🔍 Current updateTimer state:',
        mediaState.updateTimer ? 'EXISTS' : 'NULL');
    ...
    shaka.log.info(
        '🔥 BJSN/StreamingEngine DEBUG: Creating DelayedTick with delay:',
        delay);
    mediaState.updateTimer = new shaka.util.DelayedTick(async () => {
      try {
        ...
        shaka.log.error(
            '🔥 BJSN/StreamingEngine DEBUG: Error in onUpdate_():', error);
        if (this.playerInterface_) {
          this.playerInterface_.onError(error);
        }
      }
