...
shaka.media.StreamingEngine = class {
  ...
  async initStreams_(segmentPrefetchById) {
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: initStreams_ called');
    ...
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Current variant:', this.currentVariant_);
    ...
    for (const type of streamsByType.keys()) {
      ...
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Processing stream type:', type);
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Stream details:', {
        id: stream.id,
      ...
      if (!this.mediaStates_.has(type)) {
        shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Creating media state for type:', type);
        ...
        shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Scheduling initial update for type:', type);
        this.scheduleUpdate_(mediaState, 0);
      } else {
        shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Media state already exists for type:', type);
      }
    }
  }
  ...
  async onUpdate_(mediaState) {
    ...
    // BJSN DEBUG: Track onUpdate_() entry
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: onUpdate_() called for type:', mediaState.type);
    shaka.log.info('  📊 MediaState status:', {
      performingUpdate: mediaState.performingUpdate,
      updateTimer: mediaState.updateTimer ? 'EXISTS' : 'NULL',
      clearingBuffer: mediaState.clearingBuffer,
      waitingToClearBuffer: mediaState.waitingToClearBuffer,
    });
    ...
    if (mediaState.performingUpdate || (mediaState.updateTimer == null)) {
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: onUpdate_() early return - invalid state');
      return;
    }
    ...
    if (mediaState.clearingBuffer) {
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: onUpdate_() early return - clearing buffer');
      return;
    }

    mediaState.updateTimer = null;
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: onUpdate_() proceeding with update process');
  ...
  update_(mediaState) {
    ...
    // BJSN DEBUG: Track update_() calls
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: update_() called for type:', mediaState.type);
    shaka.log.info('  🎬 Stream has isAudioMuxedInVideo:', mediaState.stream.isAudioMuxedInVideo);
    ...
    if (shaka.media.StreamingEngine.isEmbeddedText_(mediaState)) {
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Embedded text detected, setting CC ID');
    ...
    if (mediaState.stream.isAudioMuxedInVideo && 
      ...
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Audio stream with muxed audio in video, skipping');
      return null;
    }
    ...
    if (!this.playerInterface_.mediaSourceEngine.isStreamingAllowed() &&
      ...
      // invalid soon.
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Streaming not allowed yet, scheduling retry');
    ...
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Getting segment reference needed');
    shaka.log.info('  📍 Presentation time:', presentationTime);
    shaka.log.info('  📍 Buffer end:', bufferEnd);
    shaka.log.info('  📍 Stream changed:', streamChanged);
    ...
    if (!reference) {
      ...
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: No segment reference found, scheduling retry');
      return updateIntervalSeconds;
    }

    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Found segment reference:', {
    ...
    if (timeNeeded >= minTimeNeeded + maxRunAhead) {
      ...
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Waiting for other streams to buffer');
    ...
    if (this.config_.crossBoundaryStrategy !== CrossBoundaryStrategy.KEEP &&
      ...
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Discarding segment due to boundary strategy');
      return null;
    }
    ...
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: About to call fetchAndAppend_');
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference, adaptation) {
    ...
    // BJSN DEBUG: Log segment fetch/append initiation
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: fetchAndAppend_ called');
    shaka.log.info('  📝 Content Type:', mediaState.type);
    shaka.log.info('  ⏰ Presentation Time:', presentationTime);
    shaka.log.info('  📊 Segment:', {
      startTime: reference.startTime,
      endTime: reference.endTime,
      uris: reference.getUris(),
  ...
  async fetch_(mediaState, reference, streamDataCallback) {
    // BJSN DEBUG: Log segment fetch operation
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: fetch_ called');
    shaka.log.info('  📝 Content Type:', mediaState.type);
    shaka.log.info('  🔗 Reference URIs:', reference.getUris());
    shaka.log.info('  📏 Reference size:', reference.getSize());
    shaka.log.info('  ⏰ Start-End time:', reference.startTime, '->', reference.endTime);

    const segmentData = reference.getSegmentData();
    if (segmentData) {
      shaka.log.info('  ✅ Using cached segment data');
      return segmentData;
    }

    let op = null;
    if (mediaState.segmentPrefetch) {
      shaka.log.info('  🔍 Checking segment prefetch');
      op = mediaState.segmentPrefetch.getPrefetchedSegment(
          reference, streamDataCallback);
    }

    if (!op) {
      shaka.log.info('  🌐 Dispatching network fetch for:', reference.getUris()[0]);
      op = this.dispatchFetch_(
          reference, mediaState.stream, streamDataCallback);
    } else {
      shaka.log.info('  ✅ Using prefetched segment');
    }
  ...
  scheduleUpdate_(mediaState, delay) {
    ...
    // BJSN DEBUG: Track all scheduleUpdate_() calls
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: scheduleUpdate_() called');
    shaka.log.info('  📝 Content Type:', mediaState.type);
    shaka.log.info('  ⏱️ Delay:', delay, 'seconds');
    shaka.log.info('  🔍 Current updateTimer state:', mediaState.updateTimer ? 'EXISTS' : 'NULL');
    ...
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: Creating DelayedTick with delay:', delay);
    mediaState.updateTimer = new shaka.util.DelayedTick(async () => {
      shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: DelayedTick fired, calling onUpdate_() for type:', mediaState.type);
      try {
        await this.onUpdate_(mediaState);
      } catch (error) {
        shaka.log.error('🔥 BJSN/StreamingEngine DEBUG: Error in onUpdate_():', error);
        if (this.playerInterface_) {
          this.playerInterface_.onError(error);
        }
      }
    ...
    shaka.log.info('🔥 BJSN/StreamingEngine DEBUG: DelayedTick created and scheduled');
  }
