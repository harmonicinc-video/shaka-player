...
shaka.media.BjsnManifestParser = class {
  ...
  constructor() {
    ...
    /** @private {number} */
    this.maxSegmentsAhead_ = 10; // Keep max 10 segments ahead of current position
  }
  ...
  async start(uri, playerInterface) {
    ...
    try {
      ...
      shaka.log.info('Successfully created BJSN manifest, isLive:', this.isLive_);
    ...
    try {
      ...
      if (isAvailable) {
        ...
        if (newBjsnData && newBjsnData.seq_num > this.lastKnownSequence_) {
          shaka.log.info('BJSN: Found new segment sequence:', newBjsnData.seq_num);
  ...
  async requestSegmentBjsnData_(segmentUrl) {
    try {
      ...
      shaka.log.warning('Failed to fetch BJSN data from segment:', segmentUrl, error);
      return null;
    }
  }
  ...
  addNewSegmentToIndex_(stream, bjsnData, sequence) {
    ...
    // Calculate segment timing based on sequence
    const baseSequence = this.currentBjsnData_.seq_num;
    const segmentIndex = sequence - baseSequence;
    const startTime = segmentIndex * this.segmentDuration_;
    const endTime = (segmentIndex + 1) * this.segmentDuration_;
    ...
    stream.segmentIndex.append([reference]);
    
    shaka.log.v2('Added new segment to stream:', stream.type, 'sequence:', sequence);
  }
  ...
  updatePresentationTimeline_() {
    ...
    const baseSequence = this.currentBjsnData_.seq_num;
    const totalSegments = (latestSequence - baseSequence) + this.maxSegmentsAhead_;
    ...
    // Update live edge
    const liveEdge = (latestSequence - baseSequence + 1) * this.segmentDuration_;
    this.presentationTimeline_.setSegmentAvailabilityStart(
        Math.max(0, liveEdge - (this.maxSegmentsAhead_ * this.segmentDuration_)));
  }
  ...
  createSegmentIndex_(bjsnData) {
    ...
    shaka.log.info('Created BJSN segment index with', references.length, 'references');
