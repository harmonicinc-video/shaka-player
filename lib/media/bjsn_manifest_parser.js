...
shaka.media.BjsnManifestParser = class {
  ...
  createSegmentIndex_(bjsnData) {
    ...
    shaka.log.info('  📊 Max segments:', maxSegments);
    ...
    for (let i = 0; i < maxSegments; i++) {
      ...
      if (this.useActualTimestamps_ && this.baseMediaTime_ !== null) {
        // Calculate timestamps based on actual MP4 timing
      ...
      shaka.log.v2('🕐 Created segment reference:', {
        sequence: currentSeqNum,
        startTime: startTime,
        endTime: endTime,
  ...
  updatePresentationTimeline_() {
    ...
    if (mediaTimings.length > 0) {
      ...
      const actualDuration = lastTiming.endTime - firstTiming.startTime + 
