...
shaka.media.BjsnManifestParser = class {
  ...
  async requestInitialSegmentWithTimingInfo_(uri) {
    ...
    shaka.log.info('🔍 BJSN PARSER: Initial segment size:', originalSegmentData.length, 'bytes');
    ...
    shaka.log.info('🔍 BJSN PARSER: Stripped segment size:', strippedSegmentData.length, 'bytes');
        ...
        .fullBox('trun', (box) => {
          if (duration === null) {
            ...
            if (parsed && parsed.totalDuration > 0) {
              ...
              shaka.log.info('  📏 TRUN total duration:', parsed.totalDuration, 'timescale units');
  ...
  parseTRUN_(reader, version, flags) {
    try {
      ...
      // If sample duration is not present per-sample, check for default duration
      if (!sampleDurationPresent) {
        ...
        for (let i = 0; i < sampleCount; i++) {
          ...
          if (sampleSizePresent) {
            ...
            if (i < 3) {
              shaka.log.info('  📦 Sample', i, 'duration:', sampleDuration, 'size:', sampleSize);
            }
          }
  ...
  createSegmentIndex_(bjsnData) {
    ...
    if (this.isLive_) {
      ...
      if (cachedTiming && 
        ...
        shaka.log.info('  ⏰ Start time:', startTime.toFixed(3), 'seconds');
        shaka.log.info('  📏 Duration:', cachedTiming.duration.toFixed(3), 'seconds');
      } else {
      ...
      shaka.log.info('  ✅ Created', maxSegments, 'VOD segments with sequential timing');
    }
  ...
  setupBjsnResponseFilter_() {
    ...
    this.bjsnResponseFilter_ = (type, response, context) => {
      ...
      if (shaka.util.BjsnBoxStripper.needsStripping(originalData)) {
        shaka.log.v2('🔧 BJSN RESPONSE FILTER: Stripping BJSN, MOOV, and FTYP boxes from segment:', response.uri);
        
        // Strip BJSN, MOOV, and FTYP boxes from media segments
        const strippedData = shaka.util.BjsnBoxStripper.stripBjsnAndMoovBoxes(originalData);
