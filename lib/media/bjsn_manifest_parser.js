...
/**
 * @summary BJSN Manifest Parser with Dynamic Codec Detection
 * Implements Shaka Player manifest parser interface for BJSN-enabled streams
 * with automatic codec detection from MP4 segments for accurate MIME types
 *
...
shaka.media.BjsnManifestParser = class {
  ...
  constructor() {
    shaka.log.info('🔥 BJSN PARSER: Constructor called - BJSN parser instantiated with codec detection');
    ...
    this.detectedCodecs_ = null; // Cache detected codec information

    /** @private {!Map<string, !shaka.util.BjsnCodecDetector.DetectionResult>} */
    this.codecCache_ = new Map(); // Cache codec detection results per segment URL

    /** @private {?shaka.extern.ResponseFilter} */
    this.bjsnResponseFilter_ = null; // Response filter for BJSN box stripping
  }
  ...
  async start(uri, playerInterface) {
    ...
    shaka.log.info('Starting BJSN manifest parser with dynamic codec detection');

    try {
      shaka.log.info('🔥 BJSN PARSER: Requesting initial segment for metadata and codec detection');
      
      // Request the initial segment to extract BJSN metadata and detect codecs
      const {bjsnData, codecInfo} = await this.requestInitialSegmentWithCodecDetection_(uri);
      ...
      // Validate detected codecs and ensure we have valid codecs
      ...
      } else if (!shaka.util.BjsnCodecDetector.validateCodecSupport(codecInfo.mimeType)) {
        shaka.log.warning('🔥 BJSN PARSER: Detected codecs not supported, using fallback');
        // Note: We continue with unsupported codecs and let MSE handle the error
        // This provides better debugging information than failing early
      }

      // Set up response filter for BJSN box stripping
      this.setupBjsnResponseFilter_();

      // Create the manifest from BJSN data with detected codecs
      ...
      shaka.log.info('Successfully created BJSN manifest with dynamic codecs, isLive:', this.isLive_);
      shaka.log.info('Detected codecs:', codecInfo);
  ...
  /**
   * Request initial segment and perform codec detection
   * @param {string} uri - The URI of the initial segment
   * @return {!Promise<{bjsnData: ?Object, codecInfo: !shaka.util.BjsnCodecDetector.CodecInfo}>} 
   *         Promise resolving to BJSN data and codec info
   * @private
   */
  async requestInitialSegmentWithCodecDetection_(uri) {
    const requestType = shaka.net.NetworkingEngine.RequestType.SEGMENT;
    const request = shaka.net.NetworkingEngine.makeRequest(
    ...
    shaka.log.info('🔥 BJSN PARSER: Cached initial segment data for reuse, size:',
    ...
    // Strip BJSN box and detect codecs from clean MP4 data
    const strippedSegmentData = shaka.util.BjsnBoxStripper.stripBjsnBox(originalSegmentData);
    
    shaka.log.info('🔍 BJSN PARSER: Starting codec detection from initial segment');
  ...
  setupBjsnResponseFilter_() {
    ...
    this.bjsnResponseFilter_ = (type, response, context) => {
      ...
      if (shaka.util.BjsnBoxStripper.needsStripping(originalData)) {
        ...
        shaka.log.v2('  📊 Size reduction:', originalData.length - strippedData.length, 'bytes');
      }
    };
...
shaka.media.BjsnManifestParser.factory = () => {
  shaka.log.info('🔥 BJSN PARSER FACTORY: Creating new BJSN parser instance with codec detection');
