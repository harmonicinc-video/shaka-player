 ...
 * with accurate timestamp parsing from MP4 segments for proper timeline
 * construction
 *
...
shaka.media.BjsnManifestParser = class {
  ...
  constructor() {
    shaka.log.info(
        '🔥 BJSN PARSER: Constructor called - BJSN parser with MP4 timestamp' +
        ' extraction');
    ...
    // Use real MP4 timestamps instead of artificial ones
    this.useActualTimestamps_ = true;
  }
  ...
  async start(uri, playerInterface) {
    ...
    shaka.log.info(
        '🔥 BJSN PARSER: start() with MP4 timestamp extraction for URI:', uri);

    try {
      // Extract both BJSN metadata and MP4 timestamps from initial segment
      const initialSegmentData =
          await this.requestInitialSegmentWithTimingInfo_(uri);
      const {bjsnData, timingInfo} = initialSegmentData;
      let {codecInfo} = initialSegmentData;
      ...
      if (timingInfo) {
        ...
        shaka.log.info(
            '🕐 BJSN TIMESTAMP: Initialized timing from first segment');
        ...
        shaka.log.info('  📊 Seg duration:', timingInfo.duration);
      }

      // Validate detected codecs
      if (!codecInfo.video && !codecInfo.audio) {
        shaka.log.warning(
            '🔥 BJSN PARSER: No codecs detected, forcing fallback');
      ...
      shaka.log.info(
          '  ⏰ Using actual timestamps:', this.useActualTimestamps_);
  ...
  /**
   * Request initial segment and extract both BJSN data and MP4 timing
   * information
   * @param {string} uri - The URI of the initial segment
   * @return {!Promise<{
   *   bjsnData: ?Object,
   *   codecInfo: !shaka.util.BjsnCodecDetector.CodecInfo,
   *   timingInfo: ?shaka.media.BjsnManifestParser.SegmentTimingInfo
   * }>}
   * @private
   */
  async requestInitialSegmentWithTimingInfo_(uri) {
    ...
    shaka.log.info(
        '🔍 BJSN PARSER: Initial segment size:',
        originalSegmentData.length, 'bytes');

    // Extract BJSN metadata
    const bjsnData =
        shaka.util.BjsnParser.parseFromSegment(originalSegmentData);
    shaka.log.info('🔍 BJSN PARSER: BJSN data extracted:', bjsnData);

    // Strip BJSN box for codec detection and MP4 parsing
    const strippedSegmentData =
        shaka.util.BjsnBoxStripper.stripBjsnBox(originalSegmentData);
    shaka.log.info(
        '🔍 BJSN PARSER: Stripped segment size:', strippedSegmentData.length,
        'bytes');

    shaka.log.info(
        '🔍 BJSN PARSER: Analyzing initial segment for codecs and timestamps');

    // Detect codecs from clean MP4 data
    const codecInfo =
        await shaka.util.BjsnCodecDetector.detectCodecsFromSegment(
            strippedSegmentData);
    shaka.log.info('🔍 BJSN PARSER: Codec detection result:', codecInfo);

    // Extract MP4 timing information
    const timingInfo = this.extractTimingFromSegment_(
        strippedSegmentData, bjsnData ? bjsnData.seq_num : 0);
    shaka.log.info('🔍 BJSN PARSER: Timing extraction result:', timingInfo);

    // Cache results
    const detectionResult =
        shaka.util.BjsnCodecDetector.createDetectionResult(uri, codecInfo);
   ...
   * @return {?shaka.media.BjsnManifestParser.SegmentTimingInfo}
   * @private
   */
  extractTimingFromSegment_(segmentData, sequenceNumber) {
    try {
      shaka.log.info(
          '🕐 BJSN TIMESTAMP: Extracting timing from segment', sequenceNumber);
      ...
      if (isInitSegment) {
        ...
        return {
          ...
          trackId: null,
          sampleCount: null,
          trackCount: this.countTracksInInit_(segmentData),
        };
      }

      // For media segments, extract TFDT and TRUN information
      shaka.log.info('  📦 Processing MEDIA segment for timestamps');

      const parser = new shaka.util.Mp4Parser()
          .box('moof', shaka.util.Mp4Parser.children)
          .box('traf', shaka.util.Mp4Parser.children)
          .fullBox('tfhd', (box) => {
          // Extract track ID from TFHD for debugging
            const flags = box.flags;
            trackId = box.reader.readUint32();
            shaka.log.info('  🎯 Found track ID:', trackId);
          })
          .fullBox('tfdt', (box) => {
            if (box.version === 0 || box.version === 1) {
              ...
              shaka.log.info('  ⏰ Timescale:', timescale);
              shaka.log.info(
                  '  🕐 Calculated startTime:', startTime, 'seconds');
            }
          })
          .fullBox('trun', (box) => {
            if (duration === null) {
            // Parse TRUN to get total segment duration
              const parsed =
                  this.parseTRUN_(box.reader, box.version, box.flags);
              if (parsed && parsed.totalDuration > 0) {
                duration = parsed.totalDuration / timescale;
                sampleCount = parsed.sampleCount;
                shaka.log.info(
                    '  📏 TRUN total duration:', parsed.totalDuration,
                    'timescale units');
                shaka.log.info('  📏 Sample count:', parsed.sampleCount);
                shaka.log.info(
                    '  🕐 Calculated duration:', duration, 'seconds');
              }
            }
          });
      ...
      if (startTime !== null) {
        ...
        shaka.log.info('  ✅ Extracted timing info:', {
        ...
        shaka.log.warning('  ⚠️ No TFDT found in segment', sequenceNumber);

        // Return fallback timing for media segments without TFDT
        return {
          startTime: 0,
          duration: duration,
          timescale: timescale,
  ...
  extractTimescaleFromInit_(initData) {
    ...
    const parser = new shaka.util.Mp4Parser()
        ...
        .fullBox('mdhd', (box) => {
          if (box.version === 0 || box.version === 1) {
            const parsed =
                shaka.util.Mp4BoxParsers.parseMDHD(box.reader, box.version);
  ...
  parseTRUN_(reader, version, flags) {
    try {
      ...
      shaka.log.info('  📊 TRUN sample count:', sampleCount);
      ...
      const sampleCompositionTimePresent = (flags & 0x000800) !== 0;

      shaka.log.info('  🏁 TRUN flags analysis:', {
        dataOffsetPresent,
        firstSampleFlagsPresent,
        sampleDurationPresent,
        sampleSizePresent,
        sampleFlagsPresent,
        sampleCompositionTimePresent,
      });

      // Skip data offset if present
      if (dataOffsetPresent) {
        const dataOffset = reader.readUint32();
        shaka.log.info('  📍 TRUN data offset:', dataOffset);
      }

      // Skip first sample flags if present
      if (firstSampleFlagsPresent) {
        const firstSampleFlags = reader.readUint32();
        shaka.log.info('  🏁 TRUN first sample flags:', firstSampleFlags);
      }

      let totalDuration = 0;
      let defaultDuration = null;

      // If sample duration is not present per-sample, check for default
      // duration
      if (!sampleDurationPresent) {
        // Use default segment duration
        defaultDuration = this.defaultSegmentDuration_ * this.timescale_;
        totalDuration = defaultDuration;
        shaka.log.info('  🔄 Using default duration per sample');
      } else {
        // Process each sample to calculate total duration
        for (let i = 0; i < sampleCount; i++) {
          ...
          if (sampleSizePresent) {
            const sampleSize = reader.readUint32();
            // Log first few samples for debugging
            if (i < 3) {
              shaka.log.info(
                  '  📦 Sample', i, 'duration:', sampleDuration,
                  'size:', sampleSize);
            }
          }
      ...
      shaka.log.info('  ✅ TRUN parsed successfully:', {
        sampleCount: sampleCount,
        totalDuration: totalDuration,
        averageDuration: totalDuration / sampleCount,
      });
  ...
  /**
   * Count the number of tracks in an init segment
   * @param {!Uint8Array} initData - MP4 init segment data
   * @return {number} Number of tracks found
   * @private
   */
  countTracksInInit_(initData) {
    let trackCount = 0;
    ...
    try {
      ...
      shaka.log.warning('Failed to count tracks in init segment:', error);
    }
  ...
  hasBox_(segmentData, boxType) {
    ...
    try {
      parser.parse(segmentData, /* partialOkay= */ true,
          /* stopOnPartial= */ true);
    } catch (error) {
  ...
  createSegmentIndex_(bjsnData) {
    ...
    shaka.log.info(
        '🕐 BJSN TIMING: Creating segment index with MP4 timestamps');
    shaka.log.info('  🔢 Starting sequence:', startSeqNum);
    shaka.log.info(
        '  ⏰ Use actual timestamps:', this.useActualTimestamps_);

    if (this.isLive_) {
      // For live streams, create the first segment with actual timestamp
      const cachedTiming = this.segmentTimingCache_.get(startSeqNum);
      let startTime;
      let endTime;

      // Check if we have valid media segment timing (not init segment)
      if (cachedTiming &&
          !cachedTiming.isInitSegment &&
          cachedTiming.startTime !== null &&
          this.useActualTimestamps_) {
        ...
        shaka.log.info(
            '  📍 Using real MP4 timestamp for first segment');
        shaka.log.info(
            '  ⏰ Start time:', startTime.toFixed(3), 'seconds');
        shaka.log.info(
            '  📏 Duration:', cachedTiming.duration.toFixed(3), 'seconds');
      } else {
        // Fallback for first segment if timing extraction failed
        startTime = 0; // Start from 0 for live streams
        endTime = this.defaultSegmentDuration_;
        shaka.log.warning(
            '  ⚠️ Using fallback timing for first segment');
        shaka.log.warning('  ⚠️ Cached timing:', cachedTiming);
      }

      const segmentUrl =
          this.generateSegmentUrl_(bjsnData.template_path, startSeqNum);
      ...
      for (let i = 0; i < maxSegments; i++) {
        ...
        if (cachedTiming &&
          ...
          // Fallback to sequential timing for VOD
        ...
        const segmentUrl =
            this.generateSegmentUrl_(bjsnData.template_path, currentSeqNum);
        const reference = new shaka.media.SegmentReference(
            startTime,
            endTime,
            () => [segmentUrl],
      ...
      shaka.log.info(
          '  ✅ Created', maxSegments, 'VOD segments with sequential timing');
    }

    shaka.log.info(
        '✅ BJSN segment index created with', references.length, 'references');
  ...
  async performUpdate_() {
    ...
    try {
      ...
      if (isAvailable) {
        ...
        const {bjsnData, timingInfo} =
            await this.requestSegmentWithTiming_(segmentUrl);

        if (bjsnData && bjsnData.seq_num > this.lastKnownSequence_) {
          shaka.log.info(
              '🕐 BJSN UPDATE: Found new segment sequence:', bjsnData.seq_num);
          ...
          if (timingInfo) {
            ...
            shaka.log.info(
                '  ⏰ Cached timing for sequence', bjsnData.seq_num, ':',
  ...
  async requestSegmentWithTiming_(segmentUrl) {
    try {
      ...
      shaka.log.warning(
          'Failed to fetch segment with timing:', segmentUrl, error);
  ...
  updateSegmentIndexesWithTiming_(bjsnData, timingInfo) {
    ...
    if (timingInfo && !timingInfo.isInitSegment) {
      ...
      endTime = startTime + timingInfo.duration;
      shaka.log.info(
          '🕐 Using extracted timing for sequence', currentSequence);
      shaka.log.info(
      ...
      startTime = (this.baseMediaTime_ || 0) +
          (segmentIndex * this.defaultSegmentDuration_);
      endTime = startTime + this.defaultSegmentDuration_;
      shaka.log.warning(
          '🕐 Using fallback timing for sequence', currentSequence);
    ...
    for (const variant of this.manifest_.variants) {
      if (variant.video) {
        this.addTimedSegmentToIndex_(
            variant.video, bjsnData, startTime, endTime);
      }
      if (variant.audio) {
        this.addTimedSegmentToIndex_(
            variant.audio, bjsnData, startTime, endTime);
      }
    }
  ...
  addTimedSegmentToIndex_(stream, bjsnData, startTime, endTime) {
    ...
    const segmentUrl =
        this.generateSegmentUrl_(bjsnData.template_path, sequence);
    ...
    shaka.log.info(
        '🕐 BJSN TIMING: Added timed segment to stream:', stream.type);
    shaka.log.info('  🔢 Sequence:', sequence);
    shaka.log.info(
        '  ⏰ Timing:', startTime.toFixed(3), '-', endTime.toFixed(3));
    shaka.log.info(
        '  📏 Duration:', (endTime - startTime).toFixed(3), 'seconds');
  }
  ...
  updatePresentationTimeline_() {
    ...
    const cachedTimings =
        Array.from(this.segmentTimingCache_.values());
    const mediaTimings = cachedTimings.filter((t) => !t.isInitSegment);

    if (mediaTimings.length > 0) {
      const sortedTimings =
          mediaTimings.sort((a, b) => a.startTime - b.startTime);
  ...
  setupBjsnResponseFilter_() {
    ...
    this.bjsnResponseFilter_ = (type, response, context) => {
      ...
      if (shaka.util.BjsnBoxStripper.needsStripping(originalData)) {
        shaka.log.v2(
            '🔧 BJSN RESPONSE FILTER: Stripping BJSN, MOOV, and FTYP boxes' +
            ' from segment:', response.uri);

        // Strip BJSN, MOOV, and FTYP boxes from media segments
        const strippedData =
            shaka.util.BjsnBoxStripper.stripBjsnAndMoovBoxes(originalData);
    ...
    shaka.log.info(
        '🔧 BJSN PARSER: Response filter registered for automatic BJSN box' +
        ' stripping');
  }
   ...
   * @param {!Object} bjsnData - BJSN metadata
   * @param {!shaka.util.BjsnCodecDetector.CodecInfo} codecInfo - Detected
   *     codec information
  ...
  createManifestFromBjsn_(bjsnData, codecInfo) {
    ...
    if (isLive) {
      ...
      if (this.baseMediaTime_ !== null) {
        ...
        initialDuration =
            Math.max(initialDuration, this.baseMediaTime_ + initialDuration);
      }
  ...
  createPeriodFromBjsn_(bjsnData, codecInfo) {
    ...
    const streams =
        this.createStreamsFromGear_(gearName, gearData, bjsnData, codecInfo);
  ...
  createStreamsFromGear_(gearName, gearData, bjsnData, codecInfo) {
    ...
    if (hasVideo) {
      ...
      if (isMuxed && hasAudio) {
        ...
        const fullMimeType = 'video/mp4; codecs="' + muxedCodecs + '"';
        videoStream.fullMimeTypes = new Set([fullMimeType]);
      } else {
        const fullMimeType = 'video/mp4; codecs="' + codecInfo.video + '"';
        videoStream.fullMimeTypes = new Set([fullMimeType]);
      }
    }
    ...
    if (hasAudio && !isMuxed) {
      ...
      audioStream = {
        ...
        fastSwitching: false,
        fullMimeTypes: new Set([
          'audio/mp4; codecs="' + codecInfo.audio + '"',
        ]),
...
shaka.media.BjsnManifestParser.factory = () => {
  shaka.log.info(
      '🔥 BJSN PARSER FACTORY: Creating new BJSN parser instance with MP4' +
...
shaka.media.ManifestParser.registerParserByMime(
    'video/mp4; codecs="avc1"', shaka.media.BjsnManifestParser.factory);
shaka.media.ManifestParser.registerParserByMime(
    'video/mp4; codecs="hvc1"', shaka.media.BjsnManifestParser.factory);
